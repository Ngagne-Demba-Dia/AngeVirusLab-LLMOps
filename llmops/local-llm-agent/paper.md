...
dans le cadre du Master Sécurité des Systèmes Embarqués. Le LLMOps pose les fondations opérationnelles
qui seront attaquées en LLM Security (LLM Security offensive). Comprendre comment
...
- **Excessive Agency détectée** : le tool `execute_command` avec guard whitelist montre
  ...
  connexion directe avec OWASP LLM08 (LLM Security).
...
Dans le Lab 1 PortSwigger (LLM Security), l'Excessive Agency a permis l'exécution

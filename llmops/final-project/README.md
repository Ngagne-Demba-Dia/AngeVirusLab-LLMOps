...
Pipeline LLM complet intégrant les 5 composants LLMOps :

| Composant | Rôle |
| --- | --- |
| LLaMA3.1:8b (Ollama) | Modèle LLM local |
| Guardrails (Input + Output) | Sécurité anti-injection |
| RAG Pipeline (ChromaDB) | Réponses ancrées dans les documents |
| LangFuse | Observabilité + traces |
...
```bash
cd AngeVirusLab-LLMOps/llmops/final-project
